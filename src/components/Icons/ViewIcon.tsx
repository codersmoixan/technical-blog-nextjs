function ViewIcon() {
	return (
		<svg data-v-64289cf8="" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" className="view-icon">
			<path
				data-v-64289cf8=""
				d="M7.90078 2.80078C4.49278 2.80078 1.74745 6.11672 0.800781 7.77469C1.74745 9.58339 4.49278 13.2008 7.90078 13.2008C11.3088 13.2008 14.0541 9.58339 15.0008 7.77469C14.0541 6.11672 11.3088 2.80078 7.90078 2.80078Z"
				stroke="currentColor"
			></path>
			<circle data-v-64289cf8="" cx="7.89922" cy="8.00078" r="2.2" stroke="currentColor"></circle>
		</svg>
	)
}

export default ViewIcon

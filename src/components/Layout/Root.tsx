...
function Root(props: RootProps) {
	const { children, backdrop = '', alt = '', animate = true, className, classes: propClasses, ...other } = props
	const classes = useStyles(props)

  return (
		<Box className={clsx(className, classes.root)} {...other}>
			{animate ? (
				<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className={classes.motion}>

...
const useStyles = makeStyles((theme: Theme) => ({
  ...
  item: {
    ...
    color: theme.colorPalette.primary.sullenGrey,
    '& .MuiTypography-body1': {
...
function CategoryList(props: CategoryListProps) {
  ...
  return (
    <Box className={clsx(className, classes.root, 'category-list')}>
      <Typography variant="body1" fontWeight={700} color={theme.palette.text.secondary}>LICENSE</Typography>
      <Typography variant="h4" className={classes.title}>{title}</Typography>
      <Box className={classes.list}>
        {list.map(item => (
          <Box className={classes.item} key={item.id}>
            <TickIcon />
            <Typography variant="body1" color="currentColor">{item.label}</Typography>
          </Box>
        ))}
      </Box>
